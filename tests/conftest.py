...
def sample_empty_comments():
    ...
    return pd.DataFrame({
        'comment_text': ['', '   ', None, 'valid comment']
    })
...
def mock_vectorizer():
    ...
    mock = MagicMock()
    mock.get_feature_names_out.return_value = np.array(['feature1', 'feature2', 'feature3'])
...
def sample_api_payload():
    ...
    return {
        "text": "This is a test comment for the API."
    }
...
def mock_gcs_client():
    ...
    mock = MagicMock()
    mock.bucket.return_value.blob.return_value.download_as_bytes.return_value = b'mock data'
    return mock
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "ml: tests machine learning"
    )
    config.addinivalue_line(
        "markers", "api: tests API"
    )
    config.addinivalue_line(
        "markers", "pipeline: tests pipeline"
    )
