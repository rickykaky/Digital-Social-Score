...
def load_prod_csv():
    if os.path.exists(PROD_CSV_PATH):
        return pd.read_csv(PROD_CSV_PATH, dtype={'id': int, 'user_id': int, 'comment_text': str, 'toxicity_score': float, 'created_at': str})
    else:
        ...
        df = df.astype({'id': int, 'user_id': int, 'comment_text': str, 'toxicity_score': float, 'created_at': str})
        return df
...
def submit_comment(payload: CommentPayload):
    ...
    print(f"INFO: Commentaire reçu - User {user_id}, Toxicity: {toxicity_score}, Social Score: {result['user_social_score']}")
...
def get_user_social_score(user_id: int):
    ...
    user_comments = prod_df[prod_df['user_id'] == user_id]
    avg_toxicity = user_comments['toxicity_score'].mean() if len(user_comments) > 0 else 0
