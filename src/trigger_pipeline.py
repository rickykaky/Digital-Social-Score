...
def evaluate_model_op(model_path: Input[Model], clean_csv_path: str) -> float:
    """Évalue le modèle."""
    import joblib
    import pandas as pd
    from sklearn.feature_extraction.text import TfidfVectorizer
...
def digital_score_pipeline(
    ...
    train_task = train_model_op(clean_csv_path=clean_csv_path)
    train_task.after(prepare_task)

    # Étape 3: Évaluation
    eval_task = evaluate_model_op(
        model_path=train_task.outputs["model_path"], clean_csv_path=clean_csv_path
    )
